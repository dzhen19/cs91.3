...
class Preparator:
    def __init__(self, load_directory='./subjects/', save_directory='./prepared_subjects/', load_file_pattern='*', save_file_name='all.npz',verbose=False, concatenate_data=True):

        self.concatenate_data = concatenate_data
    ...
    def run(self):
        ...
        for subject in progress:
            ...
            if os.path.isdir(self.load_directory + subject):
                ...
                for f in sorted(os.listdir(cur_dir)):
                    ...
                    if (not self.concatenate_data): 
                        np.savez(self.save_directory + subject, EEG=trials, labels=labels)
        ...
        if (self.concatenate_data):
            ...
            print(labels.shape)
            np.savez(self.save_directory + "all_subjects_LR", EEG=EEG, labels=labels)

    def _load_hdf5_events(self, EEG):
        # if self.verbose: print("Loading the events from the subject. ")
        # extract the useful event data
        events = pd.DataFrame()
        events['type'] = [''.join(map(chr, EEG[ref][:, 0])).strip()
                        for ref in EEG['event']['type'][:, 0]]
        #if self.verbose: print(events)
        events['latency'] = [util_func(EEG, ref)
                            for ref in EEG['event']['latency'][:, 0]]
        # if self.verbose: print(events)
        events['amplitude'] = [util_func(EEG, ref)
                            for ref in EEG['event']['sac_amplitude'][:, 0]]
        #if self.verbose: print(events)
        events['start_x'] = [util_func(EEG, ref)
                            for ref in EEG['event']['sac_startpos_x'][:, 0]]
        #if self.verbose: print(events)
        events['end_x'] = [util_func(EEG, ref)
                        for ref in EEG['event']['sac_endpos_x'][:, 0]]
        #if self.verbose: print(events)
        events['start_y'] = [util_func(EEG, ref)
                            for ref in EEG['event']['sac_startpos_y'][:, 0]]
        #if self.verbose: print(events)
        events['end_y'] = [util_func(EEG, ref)
                        for ref in EEG['event']['sac_endpos_y'][:, 0]]
        #if self.verbose: print(events)
        events['duration'] = [util_func(EEG, ref)
                            for ref in EEG['event']['duration'][:, 0]]
        #if self.verbose: print(events)
        events['avgpos_x'] = [util_func(EEG, ref)
                            for ref in EEG['event']['fix_avgpos_x'][:, 0]]
        #if self.verbose: print(events)
        events['avgpos_y'] = [util_func(EEG, ref)
                            for ref in EEG['event']['fix_avgpos_y'][:, 0]]
        # if self.verbose: print(events)
        events['endtime'] = [util_func(EEG, ref)
                            for ref in EEG['event']['endtime'][:, 0]]
